...
\tName: 'Investigate with known First/Last name'
...
Each of these categories offers different modules that request 3rd party sites after the information
has been submitted by the user. For example the application may request a target email address.

Using these classifiers, can reveal additional information that can be utilized within the application.
These classifiers may reveal telephone, physical address, or other useful data.
...
""")
    ...
    def repgen(self):
        try:
                ...
                def sorttype(feed):
                    ...
                    try:
                        if isinstance(feed, dict):
                            for feedvalue in feed.keys():
                                ReportGenerator().addheader(str(feedvalue), 2)
                                sorttype(bi.outdata[header][feedvalue])
                        elif isinstance(feed, list):
                            for feedlist in feed:
                                ReportGenerator().unorderedlist(str(feedlist))
            ...
            print("  ["+bc.CGRN+"+"+bc.CEND+"] "+bc.CYLW+"Report saved to ./skiptracer.docx\n"+bc.CEND)
        except Exception as e:
    ...
    def emailmenu(self):
        os.system('cls' if os.name == 'nt' else 'clear')
        Logo().banner()
        if bi.search_string is not None:
        ...
        try:
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("\n  [{}PROFILE{}] {}Whats the target's email address?{} [ex: username@domain.tld{}]: ".format(
                    bc.CBLU, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            bi.lookup = "email"
            print()
            if gselect == 1:
                ...
                bi.search_string = input("[{}?{}] {}Whats the target's email address?{} [ex: username@domain.tld{}]: ".format(
    ...
    def namemenu(self):
        ...
        print('\t[{}3{}] {}True People{} - {}Run name through public page of paywall{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}4{}] {}AdvancedBackgroundChecks{} - {}Run name through public page of paywall{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}5{}] {}Reset Target{} - {}Reset the name to new target{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        ...
        try:
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("[{}?{}] {}Whats the target's full name?{} [ex: Alice Smith{}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            bi.lookup = 'name'
            print()
            if gselect == 1:
    ...
    def phonemenu(self):
        ...
        print('\t[{}2{}] {}TruePeopleSearch{} - {}Run phone through public page of paid access{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        ...
        print('\t[{}6{}] {}Reset Target{} - {}Reset the Phone to new target{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}7{}] {}Back{} - {}Return to main menu{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        try:
            gselect = int(input(" [{}!{}] {}Select a number to continue:{} ".format(bc.CYLW, bc.CEND, bc.CBLU, bc.CEND)))
        except Exception:
        ...
        try:
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("[{}?{}] {}Whats the target's phone number?{} [ex: 1234567890{}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            bi.lookup = 'phone'
            print()
            if gselect == 1:
    ...
    def snmenu(self):
        ...
        print('\t[{}1{}] {}All{} - {}Run all modules associated to the screenname module group{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}2{}] {}Knowem{} - {}Run screenname to determine registered sites{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}3{}] {}NameChk{} - {}Run screenname to determine registered sites{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}4{}] {}Tinder{} - {}Run screenname and grab information if registered{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}5{}] {}Reset Target{} - {}Reset the screenname to new target{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}6{}] {}Back{} - {}Return to main menu{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        try:
            gselect = int(input(" [{}!{}] {}Select a number to continue:{} ".format(bc.CYLW, bc.CEND, bc.CBLU, bc.CEND)))
        except Exception:
        ...
        try:
            bi.lookup = 'sn'
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("[{}?{}] {}Whats the target's screenname?{} [ex: (Ac1dBurn|Zer0C00l){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            print()
            if gselect == 1:
    ...
    def platemenu(self):
        os.system('cls' if os.name == 'nt' else 'clear')
        Logo().banner()
        print(" [{}!{}] {}Plate search menu: Target info{} - {}{}".format(bc.CYLW, bc.CEND, bc.CBLU, bc.CYLW, bi.search_string, bc.CEND))
        print('\t[{}1{}] {}All{} - {}Run all modules associated to the plate module group{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}2{}] {}Plate Search{} - {}Run known vehicle plates against a database{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}3{}] {}Reset Target{} - {}Reset the plate to new target{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        ...
        try:
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("[{}?{}] {}Whats the target's plate number?{} [ex: (XYZ123|0U812){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            bi.lookup = 'plate'
            print()
            if gselect in [1, 2]:
    ...
    def domainmenu(self):
        ...
        print('\t[{}2{}] {}Subdomain Search{} - {}Get subdomains using Certificate Transparency logs{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}3{}] {}Reset Target{} - {}Reset the domain to new target{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        print('\t[{}4{}] {}Back{} - {}Return to main menu{}'.format(bc.CBLU, bc.CEND, bc.CRED, bc.CEND, bc.CYLW, bc.CEND))
        try:
            gselect = int(input(" [{}!{}] {}Select a number to continue:{} ".format(bc.CYLW, bc.CEND, bc.CBLU, bc.CEND)))
        except Exception:
        ...
        try:
            if not bi.search_string or bi.search_string in ['', None]:
                bi.search_string = input("[{}?{}] {}Whats the target's domain name?{} [ex: (victim.com|blah.net){}]: ".format(
                    bc.CRED, bc.CEND, bc.CRED, bc.CYLW, bc.CEND))
            bi.lookup = 'domain'
            print()
            if gselect in [1, 2]:
